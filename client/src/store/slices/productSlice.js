...
export const getProducts = createAsyncThunk('products/getProducts', async ({keyword='', pageNumber=0, currentCategory}) => {
    let link = `https://mern-watch-shop-server.onrender.com/api/v1/products?keyword=${keyword}&category=${currentCategory}&page=${pageNumber}`
...
export const getProductDetail = createAsyncThunk('product/getProductDetail', async(id) => {
    const res = await fetch(`https://mern-watch-shop-server.onrender.com/api/v1/product/${id}`);
    const data = await res.json();
    return data;
  }
)
